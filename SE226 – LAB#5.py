...
print("Please Enter a Number Between 3-7")
user_number = int(input())
while user_number < 3 or user_number > 7:
    user_number = int(input("invalid please enter a number between 3-7"))
rwords = random.sample(words, user_number)


def passGen(listPass):
    password = "".join(listPass)
    password: str = password[::-1]
    return password


passwordOne = passGen(rwords)


# Q2
def rep_with_upper(a):
    repLetter = random.choice(a)
    uppercase_letter = repLetter.upper()
    a = a.replace(repLetter, uppercase_letter)
    return a
...
def swap_letters(b):
    ...
    lastTwo = b[-2:]
    mid = b[2:-2]
    lastWord = lastTwo + mid + firstTwo
    return lastWord


temporaryPass = rep_with_upper(str(passwordOne))
print("first password: " + passwordOne)
lastPass = swap_letters(str(temporaryPass))
print("The finished password is :" + lastPass)


# Q3
def search_letter(key, keyLetter):
    if keyLetter in key:
        print("Positive! Searched letter have found")
        return True
    else:
        print("Negative! Searched letter couldn't have found")
        return False


search_letter(str(lastPass), "R")
